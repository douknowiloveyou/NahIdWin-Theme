![Image](https://github.com/user-attachments/assets/7906c918-cd76-4605-a506-9334c2ca2b5c)

<p align="center">
  <a href="https://github.com/douknowiloveyou/NahIdWin-Theme/stargazers"><img src="https://img.shields.io/github/stars/douknowiloveyou/NahIdWin-Theme?colorA=363a4f&colorB=b7bdf8&style=for-the-badge&logo=data:image/svg+xml;base64,"></a>
  <a href="https://discord.gg/vT33DaC3XB"><img src="https://img.shields.io/discord/1390345088021954620?style=for-the-badge&color=c6a0f6&labelColor=363a4f&logo=discord&logoColor=cad3f5"></a>
</p>

<p align="center">
  <i>You think the best VS Code theme is subjective because you haven’t seen NahIdWin</i>
</p>

<p align="center">
NahIdWin is <strong>NOT</strong> a community-driven pastel theme that aims to <strong>ACTUALLY</strong> be the middle ground between low and high-contrast themes, absolutely perfect for <strong>VIBE CODING</strong>. Written in Vanilla JSON. This theme is unfortunately not powered by AI.

</p>

### 🧠 Design Philosophy

![Image](https://github.com/user-attachments/assets/244e3a9d-86cc-4fcf-96bc-40f50b08e8a8)

### 👐 Contributing

You can contribute by starring this repo and putting the fries in the bag little bro 🥀

### 🗣️ Supported Languages

Only Python and JSON are supported for now. Feel free to submit a PR or change your code to one of the supported languages — it will be easier to switch languages than to find a color theme as good as this one fr.

### 💩 Acknowledgements

- [Oxopoopbon](https://github.com/nyoom-engineering/oxocarbon.nvim)
- [Tokyo Poop](https://github.com/folke/tokyonight.nvim)
- [Catpoopccin](https://github.com/catppuccin/nvim)
